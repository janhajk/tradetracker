...
var get = function(id, connection, callback) {
   ...
   q += ' ORDER BY connectors.name ASC, positions.aid ASC';
...
var update = function(id, values, connection, callback) {
   ...
   connection.query(q, function(err, results, fields) {
      if(err) {
         ...
         if(config.dev) console.log(fields);
         callback(null);
      }
   });
};
...
var getBTC = function(market, connection, callback) {
   ...
   var q = 'SELECT * FROM rates WHERE pair LIKE "BTC_USD" AND mid = ' + parseInt(market , 10) + ' ORDER BY timestamp DESC LIMIT 0,1';
