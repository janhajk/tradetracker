...
/**
 * Change of asset in past x seconds for market y
 *
 ...
 */
var change = function (rate, aid, cid, s, mysqlconnection, callback) {
   var cacheId = aid + cid + s;
   var cachedValue = changeCache.get(cacheId);
   if ( cachedValue !== undefined ){
      utils.log('using cached value');
      callback(null, (rate / cachedValue)-1);
      return;
   }
   ...
   mysqlconnection.query(query, function(e, rows) {
      if(e) {
         ...
         utils.log(rows);
         var change = (rate / rows[0].last)-1;
         changeCache.set(cacheId, rows[0].last);
 ...
 * @param {Function} callback
 */
var ratesRawGet = function(m, callback) {
   return (function(url, fData, keys, values, callback) {
      var uri = url();
   ...
   })(m.url, m.fData, m.keys, m.values, callback);
};
...
var all = function(mode, mysqlconnection, callback) {
   ...
   for (let i in markets) {
      tasks[i] = function(callback) {ratesRawGet(markets[i], callback);};
   }
         ...
         addAid(mergedRates, mysqlconnection, function(e, rates){
            if (e) callback(e)
               ...
               if(mode==='write') toDb(rates, mysqlconnection, function(e, rates){
                  ...
                  utils.log('adding changes of past 1h...', 'header');
                  async.each(rates, function(item, cb){
                     change(item.last, item.aid, item.cid, 3600, mysqlconnection, function(e, change){
                        utils.log('adding change for aid=' + item.aid + '; change = ' + change);
                        item.change_1h = Math.round(change*1000)/1000;
                        cb();
                     });
                  }, function(e){
                     callback(null, rates)
