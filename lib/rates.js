            ...
            addAid(mergedRates, mysqlconnection, function(e, rates){
                if (e) callback(e);
                    ...
                    if(mode==='write') toDb(rates, mysqlconnection, function(e, rates){
                        ...
                        var count = 0;
                        var total = rates.length * 2; // 1h and 24h for each
                        async.eachLimit(rates, 5, function(item, cb){
                            async.parallel([
                                function(cb2) {
                                    historical(item.last, item.aid, item.cid, 3600, mysqlconnection, function(e, hist) {
                                        count++;
                                        utils.log(count + '/' + total + ': adding 1h historical value for aid=' + item.aid + '; value = ' + hist);
                                        item.last_1h = Number(hist);
                                        cb2();
                                    });
                                },
                                function(cb2) {
                                    historical(item.last, item.aid, item.cid, 3600 * 24, mysqlconnection, function(e, hist) {
                                        count++;
                                        utils.log(count + '/' + total + ': adding 24h historical value for aid=' + item.aid + '; value = ' + hist);
                                        item.last_24h = Number(hist);
                                        cb2();
                                    });
                                }
                            ], function(e){
