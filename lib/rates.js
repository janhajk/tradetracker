...
var request = function(url, callback) {
    ...
    request(url, function(e, response, body) {
        if (e) {
            utils.log('error in request ' + tUrl, 'fatal');
            utils.log(e, 'fatal');
            callback(e);
        }
        else {
    ...
    var process = function(data, cb, retries) {
        ...
        try {
            ...
            cloudscraper.request(url, function(e, response, body) {
                if (e) {
                    utils.log('cloudscrapper failed', 'fatal');
                    cb(e);
                }
                else {
...
var historical = function(aid, cid, s, mysqlconnection, callback) {
    ...
    var query = 'SELECT last FROM (SELECT last,timestamp,aid FROM rates WHERE aid = ' + aid + ' AND timestamp > ' + (timestamp - 86400 * 2) + ') as aidTable WHERE timestamp < ' + timestamp + ' ORDER BY timestamp DESC LIMIT 0,1';
    utils.log(query);
...
var toDb = function(rates, mysqlconnection, callback) {
    ...
    mysqlconnection.query(query, function(e) {
        if (e) {
            utils.log(e, 'mysql');
            utils.log(query, 'fatal');
            utils.log('-');
            utils.log('Query length: ' + query.length + ' Bytes', 'fatal');
            utils.log('Number of rows to insert: ' + insert.length, 'fatal');
            utils.log('Row 174: ' + insert[173], 'fatal');
            callback(e);
        }
...
var all = function(mode, mysqlconnection, callback) {
    ...
    async.parallelLimit(tasks, 1, function(e, rates) {
        if (e) {
            utils.log('error in rates.all():', 'fatal');
            utils.log(e, 'fatal');
            callback(e);
        }
        else {
...
/**
 * 
 * Retrieves historical rate data for asset
 * 
 * 
 * 
 * 
 * 
 */
var historicalRange = function(aid, cid, from, to, connection, cb) {
    aid = parseInt(aid, 10);
    cid = parseInt(cid, 10);
    from = parseInt(from / 1000, 10);
    to = parseInt(to / 1000, 10);
    var query = 'SELECT * FROM rates WHERE timestamp > ' + from + ' and timestamp < ' + to + ' AND aid = ' + aid + ' AND cid = ' + cid;
    utils.log(query);
    connection.query(query, function(e, rows) {
        if (e) {
            ...
            var msg = 'no values found inbetween range for aid=' + aid;
            utils.log(msg);
            cb(msg);
        }
        else {
...
exports.historicalRange = historicalRange;
