...
var ratesRawGet = function(mysqlconnection, callback) {
   ...
   rates.request(options, function(e, data){
      ...
      if (e) {
         ...
         utils.log(data);
         callback(null, {'BTC_OKEX':data.info.btc.rights, 'LTC_OKEX':data.info.ltc.rights}, mysqlconnection);
      }
   });
};
...
var ratesGet = function(mode, mysqlconnection, callback){
   ...
   async.waterfall(asyncTasks, function (err, rates) {
            if (err) {
         console.log('Error in rateGet() of okex.js');
         console.log(err);
         callback(err);
      }
      else {
