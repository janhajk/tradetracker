...
var ratesRawGet = function(mysqlconnection, callback) {
   var md5 = require('md5');
   utils.log('String to be signed: ' + 'api_key='+ api_key + '&secret_key='+ api_secret);
   var sign = md5('api_key='+ api_key + + '&secret_key='+ api_secret).toUpperCase();
   utils.log('Sign: ' + sign);
   var rates = require(__dirname + '/../rates.js');
   var options = {
      url: baseUrl,
      method: 'POST',
      form: {api_key: api_key, sign: '40D037170129BFA95F158C39100C575A'}
   };
   utils.log(options);
   rates.request(options, function(e, data){
      ...
      if (e) {
         ...
         utils.log(data);
         callback(null, {'BTC_OKEX':data.info}, mysqlconnection);
      }
   });
};
