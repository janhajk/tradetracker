...
var ratesRaw2Object = function(ratesRaw, mysqlconnection, callback) {
   ...
   for (let i in ratesRaw) {
      ...
      pair.lowestAsk    = parseFloat(r.lowestAsk);
      pair.highestBid   = parseFloat(r.highestBid);
      pair.baseVolume   = parseFloat(r.baseVolume);
      pair.quoteVolume  = parseFloat(r.quoteVolume);
      pair.high24h      = parseFloat(r.high24hr);
      pair.low24h       = parseFloat(r.low24hr);
      pair.percentChange= parseFloat(r.percentChange);
      cRates.push(pair);
   }
...
var ratesGet = function(mode, mysqlconnection, callback){
   ...
   async.waterfall(asyncTasks, function (err, rates) {
      if (err) {
         console.log('Error in rateGet() of poloniex.js');
         console.log(err);
         callback(err);
      }
      else {
